...
## Uploaded audit findings reviewed — 17 August 2026

The uploaded GT-AUD-NASAWEB-0826 report was reconciled against the current source and live evidence. Confirmed safe repairs in this pass are: area landing CTAs now emit canonical `areaSlug` filters, legacy `area` names are normalised by the finder, physical meeting detail pages now reuse the Google map component with a single marker, public online notes suppress lines containing passwords/passcodes/meeting IDs, invalid placeholder-style phone values no longer render call actions, and public meeting/area copy now describes information as published rather than universally verified.

The following findings remain deliberately unaltered because they require organisation or provider decisions: online-group credential rotation, NAWS permission for hosted literature, transactional email/provider ownership, preview-domain/canonical strategy, final POPIA/Terms approval, Manus workspace ownership, Google Maps key restrictions, WhatsApp/helpline real-device confirmation, and source-content decisions such as meeting times, venue notes, duplicate source fields, and local treasury wording. The audit’s area-filter finding was confirmed and repaired; its static-preview snapshot is therefore stale for that issue.

### Fresh production bundle confirmation

After a cache-busted live navigation following checkpoint `173372b8`, the public finder served the new bundle: the heading reads “Search published meeting information,” the area select is Johannesburg, the result total is 98 published meetings, and the map is narrowed to the Johannesburg result set. The first uncached navigation still showed the prior bundle, so future live checks should include a cache-busting query while deployment propagation completes.
