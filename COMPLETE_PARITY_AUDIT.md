...
The refreshed public WordPress REST pass retrieved **100 published records** across the Region and all four Area sites: 78 pages, 4 posts, 1 event, 10 venues, and 7 organisers. Of those records, 79 supplied readable body content and **all 100 have an explicit migration-register outcome**, with zero unmapped records. The row-level outputs are `WORDPRESS_REST_CONTENT_CRAWL.csv`/`.json` and `REST_CONTENT_MIGRATION_COVERAGE.csv`/`.json`.

The original high-volume direct page-body crawl was limited by source throttling and timeouts. It is retained as honest transport evidence rather than used to infer missing content. Its gap is now covered by a source-appropriate coverage classification: all **1,157** sitemap URLs are categorized as WordPress REST page/post, canonical TSML meeting, canonical TSML location, legacy directory index, migration-register URL, event, or taxonomy/archive. The full classification is `SITEMAP_COVERAGE_CLASSIFICATION.csv`/`.json`; no sitemap URL is left unclassified. This establishes complete public-source inventory and routing evidence, while retaining the separate organisational approval requirement for publishing the 81 staged regional pages.
