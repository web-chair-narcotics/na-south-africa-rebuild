...
The initial high-volume HTML body crawl was throttled by the legacy server, so the audit was completed through the publicly available WordPress REST API rather than overstating failed HTML requests. The REST crawl retrieved 100 current public source records across the Region and four Areas: 78 pages, 4 posts, 1 event, 10 event venues, and 7 event organizers. Seventy-nine records contain readable source body content. All 100 REST source records have an explicit migration-register outcome, with zero unmapped live REST rows. The 1,157 sitemap URLs are fully retained and classified: REST page/post/event resources, canonical current TSML meeting/location records, and explicit legacy archive, event, media, or unmatched meeting/location buckets. Evidence is in `WORDPRESS_REST_CONTENT_CRAWL.md`, `SITEMAP_COVERAGE_CLASSIFICATION.md`, and `REST_CONTENT_MIGRATION_COVERAGE.md`.
...
## REST-based full public content coverage — 16 August 2026

The refreshed source audit now covers the complete public source inventory through the live sitemap, canonical TSML feeds, and WordPress REST collections. It does not promote stale sitemap meeting/location pages merely because their legacy URL still exists; current meeting visibility follows the canonical Area and Region TSML feed evidence already reconciled at 327/327 normalized keys. All current REST page, post, event, venue, and organizer resources are mapped in the migration register, while 81 regional content records remain intentionally staged until an authorised organisation reviewer makes the publication decision.
