...
def main() -> None:
    ...
    for source in rest["rows"]:
        key = normalize(source["source_url"])
        mapped = register_by_url.get(key)
        rows.append({
            "site": source["site"],
    ...
    output = {"summary": {"rest_rows": len(rows), "mapped": counts["mapped"], "unmapped": counts["unmapped"], "by_endpoint": dict(Counter(row["endpoint"] for row in rows))}, "rows": rows, "unmapped": unmapped}
    ...
    with (ROOT / "REST_CONTENT_MIGRATION_COVERAGE.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=["site", "endpoint", "source_url", "title", "register_match", "final_destination", "decision"])
        writer.writeheader()
