...
def main() -> None:
    ...
    covered_urls = rest_urls & sitemap_urls
    output = {
        ...
        "summary": {
            ...
            "sitemap_urls_covered_by_rest": len(covered_urls),
            "sitemap_urls_unresolved_by_rest": len(sitemap_urls - rest_urls),
        },
    ...
    markdown = [
        ...
        "| Measure | Count |",
        "|---|---:|",
        f"| Public REST records returned | {summary['rest_content_rows']} |",
        f"| Records with readable body content | {summary['rows_with_body']} |",
        f"| Sitemap URLs | {summary['sitemap_total_urls']} |",
        f"| Sitemap URLs represented by REST links | {summary['sitemap_urls_covered_by_rest']} |",
        f"| Sitemap URLs outside REST collections | {summary['sitemap_urls_unresolved_by_rest']} |",
        "",
        "## Public REST records by site",
        "",
        "| Site | Records |",
        "|---|---:|",
        *[f"| {site} | {count} |" for site, count in sorted(summary["rows_by_site"].items())],
        "",
        "## Public REST records by collection",
        "",
        "| Collection | Records |",
        "|---|---:|",
        *[f"| {endpoint} | {count} |" for endpoint, count in sorted(summary["rows_by_endpoint"].items())],
        "",
        "## Interpretation",
        "",
        "This REST crawl is the page/post source inventory for the public WordPress multisite. Sitemap URLs outside these REST collections are not automatically missing content: they include meeting and location endpoints independently covered by canonical TSML feeds, plus category, archive, media, and other non-page/post resources. The row-level results are retained in the matching CSV and JSON files.",
        "",
        "## Errors",
        "",
        *( [f"- `{key}`: {value}" for key, value in errors.items()] if errors else ["None."] ),
    ]
