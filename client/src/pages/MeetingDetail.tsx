  ...
  return <PublicLayout>
    <main className="bg-[#F8F8F8]">
      ...
      {meeting ? <section className="site-container grid gap-8 py-12 sm:py-16 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <aside className="rounded-[2rem] bg-[#54595F] p-7 text-white sm:p-10"><ShieldCheck className="h-7 w-7 text-[#2F9B3E]" aria-hidden="true" /><h2 className="mt-6 font-serif text-3xl">Plan your visit</h2><p className="mt-4 leading-7 text-[#F8F8F8]">Confirm the meeting details and open the exact route before you leave. If you need another option, return to the finder to search by day, area, or format.</p><div className="mt-8 grid gap-3">{meeting.meetingFormat === "online" && meeting.onlineUrl ? <a className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[#2F9B3E] px-4 font-bold text-white hover:bg-[#EEEEEE]" href={meeting.onlineUrl} target="_blank" rel="noreferrer">Join online <ExternalLink className="ml-2 h-4 w-4" aria-hidden="true" /></a> : meeting.meetingFormat !== "online" && (meeting.latitude || meeting.streetAddress) ? <a className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[#2F9B3E] px-4 font-bold text-white hover:bg-[#EEEEEE]" href={directionsUrl(meeting)} target="_blank" rel="noreferrer">Get exact Google Maps directions <ExternalLink className="ml-2 h-4 w-4" aria-hidden="true" /></a> : null}<Link className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/30 px-4 font-bold text-white hover:bg-white/10" href="/meetings"><Users className="mr-2 h-4 w-4" aria-hidden="true" />Find another meeting</Link></div></aside>
      </section> : null}
    </main>
  </PublicLayout>;
}
