  ...
  return <PublicLayout>
    <main className="bg-[#F8F8F8]">
      ...
      {meeting ? <section className="site-container grid gap-8 py-12 sm:py-16 lg:grid-cols-[1.1fr_.9fr]">
        <div className="rounded-[2rem] border border-[#EEEEEE] bg-white p-7 shadow-[0_8px_24px_rgba(20,45,42,0.04)] sm:p-10"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-[#F8F8F8] px-3 py-1 text-sm font-bold text-[#2F9B3E]">{meeting.meetingType}</span><span className="rounded-full bg-[#F8F8F8] px-3 py-1 text-sm font-bold text-[#7A7A7A]">{meeting.meetingFormat.replace("_", " ")}</span></div><div className="mt-8 grid gap-6"><div className="flex gap-4"><Clock3 className="mt-1 h-5 w-5 shrink-0 text-[#085C84]" aria-hidden="true" /><div><h2 className="font-bold text-[#54595F]">When</h2><p className="mt-1 leading-7 text-[#7A7A7A]">{readDays(meeting.daysOfWeek)} · {meeting.startTime}</p></div></div><div className="flex gap-4"><MapPinned className="mt-1 h-5 w-5 shrink-0 text-[#085C84]" aria-hidden="true" /><div><h2 className="font-bold text-[#54595F]">{meeting.meetingFormat === "online" ? "Online access" : "Where"}</h2><p className="mt-1 leading-7 text-[#7A7A7A]">{meeting.meetingFormat === "online" ? "Online meeting — use the verified join link in the action panel." : addressOf(meeting) || "Location details available from the area."}</p></div></div>{meeting.contactPerson || meeting.phone ? <div className="flex gap-4"><Phone className="mt-1 h-5 w-5 shrink-0 text-[#085C84]" aria-hidden="true" /><div><h2 className="font-bold text-[#54595F]">Contact</h2>{meeting.contactPerson ? <p className="mt-1 leading-7 text-[#7A7A7A]">{meeting.contactPerson}</p> : null}{meeting.phone ? <a className="mt-1 inline-flex font-bold text-[#2F9B3E] underline underline-offset-4" href={`tel:${meeting.phone.replace(/\s/g, "")}`}>{meeting.phone}</a> : null}</div></div> : null}{meeting.specialNotes ? <div className="rounded-2xl bg-[#F8F8F8] p-5 leading-7 text-[#7A7A7A]"><h2 className="font-bold text-[#54595F]">Meeting notes</h2><p className="mt-2 whitespace-pre-line">{meeting.specialNotes}</p></div> : null}</div></div>
