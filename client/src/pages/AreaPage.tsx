  ...
  return (
    <PublicLayout>
      <main className={`area-page ${area.className}`}>
        <section className="relative overflow-hidden bg-[#142d2a] py-16 text-white sm:py-24">
          ...
          <div className="site-container relative grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
            <div>
              <p className="eyebrow text-[#bcd6cf]">{area.eyebrow}</p>
              <h1 className="mt-4 max-w-3xl font-serif text-5xl leading-[1.02] tracking-[-0.05em] sm:text-7xl">{area.title}</h1>
              <p className="mt-7 max-w-2xl text-lg leading-8 text-[#e5ede8]">{area.summary}</p>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button asChild className="min-h-12 rounded-xl bg-[#c7d9ae] px-5 font-bold text-[#142d2a] hover:bg-[#dce9c8]"><Link href={finderHref}>Find a {area.name} meeting <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" /></Link></Button>
        ...
        <section className="site-container grid gap-8 py-14 sm:py-20 lg:grid-cols-[1fr_.8fr]">
          <div><p className="eyebrow">The local directory</p><h2 className="mt-3 max-w-2xl font-serif text-4xl leading-tight tracking-[-0.04em] text-[#142d2a]">A practical way to take the next step.</h2><p className="mt-6 max-w-2xl text-lg leading-8 text-[#405057]">{area.detail}</p><Link href={finderHref} className="mt-8 inline-flex min-h-11 items-center gap-2 font-bold text-[#17658b] underline decoration-[#9ec5d5] underline-offset-4">Open the {area.name} directory <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div>
          <aside className="rounded-[2rem] bg-[#e6eee3] p-7 sm:p-9"><MapPinned className="h-6 w-6 text-[#2e6756]" aria-hidden="true" /><h2 className="mt-5 font-serif text-3xl text-[#142d2a]">Before you go</h2><p className="mt-4 leading-7 text-[#405057]">Meeting information can change. Check the current result, venue, time, and directions before travelling. If you need immediate help, call the national phoneline.</p><a href="tel:+27861006962" className="mt-6 inline-flex min-h-11 items-center gap-2 font-bold text-[#142d2a] underline underline-offset-4">Call 0861 00 6962 <ArrowRight className="h-4 w-4" aria-hidden="true" /></a></aside>
        </section>
      </main>
    </PublicLayout>
  );
