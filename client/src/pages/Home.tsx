        ...
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          ...
          <div className="grid gap-4 sm:auto-rows-fr sm:grid-cols-3">
            {supportSteps.map(step => {
              ...
              const content = <><div className="flex items-center justify-between"><span className="font-mono text-sm font-bold text-[#085C84]">{step.number}</span><Icon className="h-5 w-5 text-[#2F9B3E]" aria-hidden="true" /></div><h3 className="mt-8 text-lg font-bold text-[#54595F]">{step.title}</h3><p className="mt-2 text-sm leading-6 text-[#7A7A7A]">{step.text}</p><span className="mt-auto pt-5 inline-flex items-center font-bold text-[#387CBB] underline underline-offset-4">Take this step <ArrowRight className="ml-1 h-4 w-4" aria-hidden="true" /></span></>;
              return step.external ? <a key={step.number} href={step.href} className="flex min-h-[276px] h-full flex-col rounded-2xl border border-[#EEEEEE] bg-white p-5 shadow-[0_8px_24px_rgba(20,45,42,0.04)] transition-shadow hover:shadow-[0_12px_30px_rgba(20,45,42,0.10)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#387CBB]">{content}</a> : <Link key={step.number} href={step.href} className="flex min-h-[276px] h-full flex-col rounded-2xl border border-[#EEEEEE] bg-white p-5 shadow-[0_8px_24px_rgba(20,45,42,0.04)] transition-shadow hover:shadow-[0_12px_30px_rgba(20,45,42,0.10)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#387CBB]">{content}</Link>;
            })}
          </div>
        </div>
      </section>
