...
The managed-storage responses for all five supplied WebP assets were checked and each resolves to HTTP 200 with `image/webp`. The Area and Region route implementation was strengthened so each hero uses an explicit eager-loaded `<img>` element, `fetchPriority="high"`, and deterministic stacking beneath the contrast overlay and content; it no longer relies only on a CSS background declaration. Fresh desktop and 375 × 812 captures visibly show the correct distinct hero image on the South Africa Region, Johannesburg, Cape Town, Pretoria, and KwaZulu-Natal routes. The dedicated hero regression, full 46-test suite, TypeScript validation, and production build all pass. Live-domain browser probing from the sandbox produced an intermittent TLS/middlebox failure after a successful text retrieval, so the post-publish production confirmation is retained as a separate final check.
