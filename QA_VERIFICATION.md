...
## Rollback recovery verification — 16 August 2026

The project was restored from the five-site visual rollback to the latest stable platform state, recovering the online/in-person separation, format-aware legacy redirects, public source-content routing, and the supplied-logo implementation. The full-bleed five-site hero presentation was then reapplied on top of that recovered state. Fresh desktop captures confirm the explicit online no-venue/no-map/no-directions experience, the physical finder’s directions-oriented experience, and five visually distinct Region, Johannesburg, Cape Town, Pretoria, and KwaZulu-Natal routes. The route checker returned HTTP 200 for all primary public pages, both finder filters, both Western Cape legacy meeting directories, the five site routes, a meeting detail route, and the administration entry route. The recovery verification completed with 46 passing tests across 15 files, TypeScript validation, and the production build.

## Refreshed full public-source coverage — 16 August 2026

The five WordPress REST sites returned 100 current public content records: 78 pages, 4 posts, 1 event, 10 venues, and 7 organisers. The migration register has an explicit outcome for all 100 records. In parallel, the complete 1,157-URL sitemap inventory was reclassified against the REST records, canonical TSML meeting and location feeds, legacy directory indexes, the URL migration register, event routes, and archive routes. Every sitemap URL has a documented coverage class. This completes source inventory and migration-routing evidence; it does not override the separate organisation-owned decisions to publish staged regional pages or approve individual legacy records.

## Five-site live-hero rendering correction — 16 August 2026

The managed-storage responses for all five supplied WebP assets were checked and each resolves to HTTP 200 with `image/webp`. The Area and Region route implementation was strengthened so each hero uses an explicit eager-loaded `<img>` element, `fetchPriority="high"`, and deterministic stacking beneath the contrast overlay and content; it no longer relies only on a CSS background declaration. Fresh desktop and 375 × 812 captures visibly show the correct distinct hero image on the South Africa Region, Johannesburg, Cape Town, Pretoria, and KwaZulu-Natal routes. The dedicated hero regression, full 46-test suite, TypeScript validation, and production build all pass. After publication, each of the five public production routes was retrieved with a cache-busting query and exposed its distinct managed hero WebP URL and correct alternative text in the public document. A live-browser screenshot attempt intermittently encountered an external TLS/middlebox error and then a browser crash loop, but the public route and direct image markup checks completed for all five routes.

## Production asset delivery root-cause repair — 16 August 2026

The user’s live-browser screenshot confirmed that the logo was genuinely broken and that the supplied site imagery could not complete in production. The direct cause was the production Content Security Policy: `img-src` admitted only same-origin and Google image sources, while `/manus-storage/*` is intentionally a same-origin proxy that returns a 307 redirect to a signed off-origin HTTPS storage URL. Browsers therefore followed the proxy redirect but blocked the final image. The policy now permits HTTPS image targets while retaining restrictive defaults for scripts, connections, frames, forms, and objects. A new regression test asserts this required contract alongside explicit image elements for the logo and five Area/Region routes. Verification before publication: 48 tests across 16 files, TypeScript, and production build pass.

The storage route has now been made independent of browser redirect and CSP behavior: it retrieves the short-lived signed object server-side and returns the exact image bytes from the existing same-origin `/manus-storage/*` URL. Local verification returned `200 OK` and the correct `image/png` or `image/webp` content type and non-zero byte length for the shared logo and representative Region and Johannesburg hero assets. This removes the off-origin image request entirely while keeping the signed URL server-side.

The prior production asset audit was invalidated by the user’s direct browser result. The exact public documents contained the old managed-storage identifiers, while the public browser transport also intermittently returned a TLS protocol error. The original logo and five approved hero files have now been re-uploaded with new production identifiers: logo `601526a4`, Region `52d58af0`, Johannesburg `e0446498`, Cape Town `2023d4b9`, Pretoria `f804b9ea`, and KwaZulu-Natal `44bd7fa1`. Local desktop/mobile captures show all six newly referenced images visibly rendering. Final exact-public-route verification is intentionally pending until this new deployment is retrieved from the user-facing browser.

## Post-fix production quality audit — 16 August 2026

The fresh production audit confirms that the home page logo is complete with its expected 1200 × 300 intrinsic dimensions and no broken image elements. All five site routes render their distinct full-bleed supplied hero image with visible desktop and mobile crops. Primary public routes, both finder formats, two representative Western Cape legacy meeting directories, a meeting-detail route, and the administration entry all returned HTTP 200.

The public finder continues to preserve the required journey separation. The online route shows 51 verified online results with join/contact actions and expressly excludes venue, map, and directions treatment. The in-person route shows 255 verified results with venue information, map controls, and Google Maps directions actions. The legacy Western Cape online directory resolves to the online-only finder with that behavior intact. The only open items remain the previously documented organisation-owned content-publication, email-provider, authorised-account, and native-device acceptance gates.

## Mobile meeting finder repair — 16 August 2026

The supplied mobile screenshots identified three technical defects: physical addresses were rendered inside a shrinking flex row, descriptions exposed raw HTML entities, and online cards carried area labels. The repair makes the public finder default to `in_person`, keeps the online route separately addressable by its dedicated button/query, hides area badges on online-only cards, decodes common HTML entities, and renders addresses in a minimum-width block with natural word wrapping. Full 375 × 812 captures now show readable in-person address blocks, decoded `&` text, no area badge on online cards, no online physical address/map/directions content, and separate join/contact actions. Regression coverage increased to 49 passing tests; TypeScript and production build pass.

Online meeting detail pages were also corrected: they now use an `Online meeting` eyebrow rather than an area label and use an online-access icon rather than a physical map-pin icon. Physical meeting detail pages retain their area, venue, and directions presentation. The added detail contract is covered by the passing regression suite. After the index no-store deployment repair, the fresh public production retrieval of `/meetings/2` now returns `Online meeting`, `Online access`, and no `Johannesburg area` label.

## Final Region Multi-Site specification implementation baseline — 16 August 2026

The final user specification is represented directly in the implementation and task ledger, while its live mobile and desktop pass/fail requirement remains the completion standard. The home page’s three pathway cards now use an equal-height desktop grid and bottom-aligned actions. Full mobile finder captures show discrete in-person and online result sets: physical cards use readable multi-line address blocks and directions, while online cards show no physical address, map, directions, or area badge.

The previous map failure is now protected in two layers. The proxied Google Maps loader retries transient initialization failures twice. If it still cannot initialize, the page renders a usable embedded Google Maps fallback centred on the first valid meeting coordinate, with a one-tap full-map link and manual retry; it does not leave visitors at a dead-end text panel. The current in-person finder successfully loaded the proxied interactive map in the development browser.

The Literature placeholder has been replaced by a catalogue with the seven titles named on the South Africa source page and 40 verified official NA World Services public resources: eight booklets, 25 informational pamphlets, and seven group readings. The page links to official NA World Services eLit destinations rather than copying or re-hosting protected works. `LITERATURE_SOURCE_INVENTORY.md` records each authoritative source and destination. Full regression, TypeScript, and production-build validation pass with 55 tests. No final instruction item is marked complete until it passes the required live desktop and mobile checks after publication.
